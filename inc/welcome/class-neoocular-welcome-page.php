...
if ( ! class_exists( 'Neoocular_Welcome_Page' ) ) {
	/**
	 * Welcome page theme class
	 */
	class Neoocular_Welcome_Page {
		private static $instance;

		/**
		 * @return Neoocular_Welcome_Page
		 */
		...
		function init_activation_hook() {
			if ( ! is_network_admin() ) {
				set_transient( '_neoocular_welcome_page_redirect', 1, 30 );
			}
		}
		...
		function welcome_page_redirect() {
			...
			if ( ! get_transient( '_neoocular_welcome_page_redirect' ) ) {
				return;
			}

			// delete the redirect transient
			delete_transient( '_neoocular_welcome_page_redirect' );
			...
			wp_safe_redirect( add_query_arg( array( 'page' => 'neoocular_welcome_page' ), esc_url( admin_url( 'themes.php' ) ) ) );
			exit;
		}
		...
		function create_welcome_page() {
			add_theme_page(
				esc_html__( 'About', 'neoocular' ),
				esc_html__( 'About', 'neoocular' ),
				current_user_can( 'edit_theme_options' ),
				'neoocular_welcome_page',
				array( $this, 'welcome_page_content' )
			);

			remove_submenu_page( 'themes.php', 'neoocular_welcome_page' );
		}
		...
		function welcome_page_content() {
			...
			$params['theme_screenshot']  = file_exists( NEOOCULAR_ROOT_DIR . '/screenshot.png' ) ? NEOOCULAR_ROOT . '/screenshot.png' : NEOOCULAR_ROOT . '/screenshot.jpg';

			neoocular_template_part( 'welcome', 'templates/welcome', '', $params );
		}
		...
		function enqueue_styles( $hook ) {
			...
			if ( 'appearance_page_neoocular_welcome_page' === $hook ) {
				wp_enqueue_style( 'neoocular-welcome-page-style', NEOOCULAR_INC_ROOT . '/welcome/assets/admin/css/welcome.min.css' );
			}
		}
	}

	Neoocular_Welcome_Page::get_instance();
}
