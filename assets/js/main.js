	...
	var qodefSwiper = {
		...
		getOptions: function ( $holder, returnBreakpoints ) {
			...
			var options = {
				...
				pagination: { el: pagination, type: 'fraction', clickable: true,
...
(function ( $ ) {
	...
	var qodefWooSelect2 = {
		init: function ( settings ) {
			...
			this.holder.push(
				{
					holder: $( '.qodef-appointment-form .menu-dd select' ),
					options: {
						minimumResultsForSearch: Infinity
					}
				}
	...
	$( document ).on(
		...
		function () {
			...
			if ( typeof qodef.qodefMagnificPopup === 'object' ) {
				...
				var $item = $( '.qodef--single.qodef-magnific-popup.qodef-popup-gallery .woocommerce-product-gallery__image' );
				if ( $item.length ) {
